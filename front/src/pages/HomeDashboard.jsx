...
export default function HomeDashboard({ user, onNavigate, onViewDetail }) {
  ...
  const periodDays = weekFilter === 'Últimos 14 días' ? 14 : 7;
  const periodFollowUps = filterFollowUpsByPeriod(followUps, today, periodDays);
  const weeklyVisitsCount = periodFollowUps.length;
  const weeklyData = buildWeeklyVisitsChart(followUps, today, periodDays);
  ...
  const achievements = [];
  if (followUps.length >= 10) achievements.push({ icon: 'military_tech', label: '10+ Seguimientos', color: 'var(--color-primary)' });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-stack-lg)' }}>
      ...
      <div className="bento-grid">
        ...
        <section id="weekly-summary" className="card weekly-summary" style={{ gridColumn: 'span 8', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div className="stat-cards-grid">
            <div className="stat-card" style={{ backgroundColor: 'rgba(0, 90, 113, 0.05)', border: '1px solid rgba(0, 90, 113, 0.1)' }}>
              ...
              <div style={{ fontSize: '28px', fontWeight: 800, color: 'var(--color-primary)' }}>{weeklyVisitsCount}</div>
              <div className="stat-card__label">Visitas Realizadas</div>
            </div>
            <div className="stat-card" style={{ backgroundColor: 'rgba(75, 100, 80, 0.08)', border: '1px solid rgba(75, 100, 80, 0.15)' }}>
              ...
              <div className="stat-card__label">Voluntarios Activos</div>
            </div>
        ...
        <section className="card" style={{ gridColumn: 'span 4', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <button
            ...
            onClick={() => onNavigate('pacientes', { sectionId: 'patients-directory' })}
          >
            Ver Agenda Completa
          </button>
        </section>
        ...
        <section className="card" style={{ gridColumn: 'span 4', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <button className="btn btn-tertiary" style={{ width: '100%', justifyContent: 'flex-start', gap: '12px', height: '48px', paddingLeft: '16px' }} onClick={() => onNavigate('estadisticas', { sectionId: 'monthly-activity-chart' })}>
        ...
        <section className="card" style={{ gridColumn: 'span 8', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button className="btn btn-tertiary" style={{ height: '32px', padding: '0 12px', fontSize: '13px', fontWeight: 600 }} onClick={() => onNavigate('pacientes', { sectionId: 'patients-directory' })}>
              Ver todos
            </button>
          </div>
