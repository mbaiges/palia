...
- El flujo OAuth de browser valida state/nonce (y PKCE si el flujo con redirect lo requiere), issuer, audience, expiración y correo verificado; la cookie opaca es el único mecanismo de sesión para rutas protegidas.
...
- OAuth de navegador: claims/`email_verified`, `state`/nonce, callback, bootstrap de primer admin y aislamiento del verificador fake de test, conforme al contrato ya acordado.
